...
export default function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    products = products.filter((product) => {
      ...
      const matchesShape =
        shape.length === 0 ||
        shape.includes(product["Shape"]?.toString().toLowerCase().trim());
      const matchesColor =
        color.length === 0 || color.includes(product["Color"]?.toLowerCase());
      const matchesClarity =
        clarity.length === 0 ||
        clarity.includes(product["Clarity"]?.toLowerCase());
      const matchesPrice =
        product["Diamond Parcel Price"] >= minPrice &&
        product["Diamond Parcel Price"] <= maxPrice;
      const matchesCarat =
